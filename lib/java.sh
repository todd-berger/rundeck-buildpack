...
JDK_URL_1_9="https://af-ops.siri.apple.com/artifactory/orchard-languages/java/versions/openjdk1.9-latest.tar.gz"
JDK_URL_1_8="https://af-ops.siri.apple.com/artifactory/orchard-languages/java/versions/oracle-jdk-1.8.0_51.tar.gz"
JDK_URL_1_7="https://af-ops.siri.apple.com/artifactory/orchard-languages/java/versions/oracle-jdk-1.7.0_45.tar.gz"
JDK_URL_1_6="https://af-ops.siri.apple.com/artifactory/orchard-languages/java/versions/oracle-jdk-1.6.0_65.tar.gz"
JDK_URL_1_6_DARWIN="https://af-ops.siri.apple.com/artifactory/orchard-languages/jdk/openjdk6-u33-heroku-temaki-darwin.tar.gz"
JDK_URL_1_7_DARWIN="https://af-ops.siri.apple.com/artifactory/orchard-languages/jdk/openjdk7-u4-heroku-temaki-darwin.tar.gz"
...
get_jvm_utils_command() {
  ...
  if [ ! -f "$jvmUtilsDir"/jvm-utils.jar ] ; then
    jvmUtilsURL="https://af-ops.siri.apple.com/artifactory/orchard-languages/jdk/buildpack-jvm-utils-0.1.jar"
...
export PATH="\$HOME/.profile.d/bin:\$JAVA_HOME/bin:\$PATH"
limit=\$(ulimit -u)
case \$limit in
256)   # 1X Dyno
...
_install_tools() {
  ...
  mkdir -p $ctxDir/.profile.d/bin
  mkdir -p $ctxDir/.profile.d/with_jmap/bin
  cp $curDir/opt/with_jmap $ctxDir/.profile.d/bin/with_jmap
  cp $curDir/opt/with_jmap_java $ctxDir/.profile.d/with_jmap/bin/java
  chmod +x $ctxDir/.profile.d/bin/with_jmap
  chmod +x $ctxDir/.profile.d/with_jmap/bin/java

  mkdir -p $ctxDir/.profile.d/with_jstack/bin
  cp $curDir/opt/with_jstack $ctxDir/.profile.d/bin/with_jstack
  cp $curDir/opt/with_jstack_java $ctxDir/.profile.d/with_jstack/bin/java
  chmod +x $ctxDir/.profile.d/bin/with_jstack
  chmod +x $ctxDir/.profile.d/with_jstack/bin/java

  cp $curDir/opt/java $ctxDir/.profile.d/bin/java
  chmod +x $ctxDir/.profile.d/bin/java
}
